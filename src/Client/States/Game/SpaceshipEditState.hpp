...
namespace ewn
{
	class SpaceshipEditState final : public AbstractState
	{
		public:
			...
			inline SpaceshipEditState(CreateMode mode, StateData& stateData, std::shared_ptr<Ndk::State> previousState, std::string hullModelPath, Nz::UInt32 hullId);
			inline SpaceshipEditState(EditMode mode, StateData& stateData, std::shared_ptr<Ndk::State> previousState, std::string spaceshipName);
		...
		private:
			...
			void UpdateSpaceshipModel(const std::string& hullModelPath);
			...
			std::string m_spaceshipModelPath;
